...
public class Info {
	...
	System.out.print(Info.getBetweenDayNumber("2012-11-11 11:19", "2012-11-11 11:11"));
	 ...
	 public static String getselect(String name,String tablename,String zdname,String type)
	 {
		 String select = "<select name=\""+name+"\" >";
	 ...
	 public static String getradio(String name,String tablename,String zdname)
	 { 
		 String radio="";
		 for(HashMap permap:new CommDAO().select("select * from "+tablename+" order by id desc")){ 
	 ...
	 public static void writeExcel(String fileName,String prosstr,java.util.List<List> list,HttpServletRequest request, HttpServletResponse response){    
	        ...
	        try {
				response.sendRedirect("/fzsalessys/upload?filename="+fileName.substring(fileName.lastIndexOf("/")+1));
			} catch (IOException e) {
				e.printStackTrace();
			}
	 ...
	 public static String getFileUpInfo()
		{
			String jscode = "";
			jscode+="<script src=/fzsalessys/js/popup.js></script>";
			jscode+="<font onclick=\"uploaddoc()\" src=\"/fzsalessys/js/nopic.jpg\" style='cursor:hand' id=txt >点击此处上传</font>";
	  ...
	  public static String tform(HashMap map)
		{
	    	...
	    	Set set = map.entrySet();
	    	 ...
	    	 jscode+=" if(f1."+str1+")\n";
	    	  ...
	    	  jscode+="{\n";
	    	  jscode+="document.getElementById(\"txt\").src=\"/fzsalessys/upfile/"+map.get("filename")+"\";\n";
	    	  jscode+="}\n";
	...
	{
		String jscode = "";
		jscode+="<img style=\"cursor: hand\" onclick=\"uploadimg()\" src=\"/fzsalessys/js/nopic.jpg\" id=txt height=\""+height+"\"/>";
		jscode+="<input type=hidden name=\"filename\" id=\"filename\" value=\"\" />";
		jscode+="<script type=\"text/javascript\"  src=\"/fzsalessys/js/popups.js\"></script>";
		return jscode;
	}
	...
	public static String getImgUpInfo2(int height)
	{
		...
		jscode+="<img style=\"cursor: hand\" onclick=\"uploadimg()\" src=\"/fzsalessys/js/nopic.jpg\" id=txt height=\""+height+"\"/>";
		jscode+="<input type=hidden name=\"filename\" id=\"filename\" value=\"\" />";
		jscode+="<script type=\"text/javascript\"  src=\"/fzsalessys/js/popup.js\"></script>";
		return jscode;
	}
	...
	public static String fck(int height,String content)
	{
		...
		jscode+="<script type=\"text/javascript\"  src=\"/fzsalessys/fckeditor/fckeditor.js\"></script>";
		jscode+="<script language=\"javascript\">";
		jscode+="function fckinit()";
		jscode+="{";
		jscode+=" var of = new FCKeditor(\"content\");";
		jscode+="of.BasePath=\"/fzsalessys/fckeditor/\";";
